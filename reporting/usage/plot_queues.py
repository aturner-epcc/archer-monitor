...
# List of log files
files = get_filelist(indir, 'queues')
...
for curperiod in repperiod:
    ...
    for file in files:
        ...
        if fdate >= startfile:
            ...
            (dates, usage) = compute_multiple_timeline(ncol, intervals[curperiod], file, scale=1./60)
            alldates.extend(dates)
            for i in range(ncol):
                allusage[i].extend(usage[i])

    imgfile = "{0}/{1}_{2}.png".format(outdir, filestem, curperiod)
    titles = ['Running', 'Queued', 'Held']
    plot_multiple_timeline(alldates, allusage, startdate, now, 'Node Hours', titles, imgfile)
